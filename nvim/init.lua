...
for _, pkg in ipairs(pkgs) do
  MiniDeps.add({ source = pkg.source, depends = pkg.depends })
  if pkg.now then MiniDeps.now(pkg.now) end
  if pkg.later then MiniDeps.later(pkg.later) end
end
