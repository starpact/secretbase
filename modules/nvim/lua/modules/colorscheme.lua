...
local function generate(p, opt)
  ...
  return lush(function(injected_functions)
    ...
    return {
      Normal          { bg = not opt.transparent_background and p.bg or "NONE", fg = p.fg },
      ...
      Search          { bg = p.blossom.lightness(p.bg.l + 24), fg = p.fg }, -- Last search pattern highlighting (see 'hlsearch').  Also used for similar items that need to stand out.
      IncSearch       { bg = p.blossom.lightness(p.bg.l + 56), fg = p.bg, gui = "bold" }, -- 'incsearch' highlighting; also used for the text replaced with ":s///c"
      ...
      NonText         { fg = p1.bg.li(30) }, -- '@' at the end of the window, characters from 'showbreak' and other characters that do not really exist in the text (e.g., ">" displayed when a double-wide character doesn't fit at the end of the line). See also |hl-EndOfBuffer|.
...
local specs = generate(palette, { transparent_background = true })
lush(specs)
